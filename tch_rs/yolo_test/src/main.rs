...
use tch::Tensor;
const CONFIG_NAME: &str = "./yolo-v3.cfg";
...
fn draw_rect(t: &mut Tensor, x1: i64, x2: i64, y1: i64, y2: i64) {
    let color = Tensor::from_slice(&[0., 0., 1.]).view([3, 1, 1]);
    t.narrow(2, x1, x2 - x1).narrow(1, y1, y2 - y1).copy_(&color)
}

fn report(pred: &Tensor, img: &Tensor, w: i64, h: i64) -> Result<Tensor> {
...
fn main() -> Result<()> {
    ...
    // ensure!(args.len() >= 3, "usage: cargo run ./yolo-v3.ot ./dog.jpg ...");
    ...
    // 下載yolo-v3.ot：https://github.com/LaurentMazare/ocaml-torch/
    // releases/download/v0.1-unstable/yolo-v3.ot
    vs.load(&args[1])?;
    println!("載入{}完成", &args[1]);
